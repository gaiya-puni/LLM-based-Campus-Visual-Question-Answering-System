...
`build_semantic_index.py` 会根据 `scene_profiles.json` 和所有 `*_pois.json` 生成本地语义向量索引；只有 POI 或场景语料变化时才需要运行。
...
在后端目录运行一次 `python build_scene_heatmaps.py` 后重启后端，在智能问答页面选择“场景热图”。
