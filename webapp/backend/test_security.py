...
class SecurityTests(unittest.TestCase):
    ...
    def test_emotions_are_returned(self):
        messages = [{'id': 1, 'owner': 'tester', 'slogan': 'hello'}]
        with patch.object(server, '_EMOTIONS', messages):
            response = self.client.get('/api/emotions')
        self.assertEqual(response.status_code, 200)
